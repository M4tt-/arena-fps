...
func _apply_hover_only(_delta: float) -> void:
	...
	velocity.y = y_error / max(get_physics_process_delta_time(), 0.001)

func _apply_hover_and_strafe(_delta: float) -> void:
	# Hover
	_apply_hover_only(_delta)

	# Strafe left/right along the enemy’s local X axis
	...
	velocity.x = x_error / max(get_physics_process_delta_time(), 0.001)
